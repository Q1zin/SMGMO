...
def plot_mse_vs_degree():
    ...
    for ax, (f, fname, eps) in zip(axes, functions):
        ...
        ax.axvspan(1, 2, alpha=0.1, color='orange', label='недообучение')
        ax.axvspan(3, 6, alpha=0.1, color='green', label='норма')
        ax.axvspan(10, 20, alpha=0.1, color='red', label='переобучение')
        ax.set_yscale('log')
