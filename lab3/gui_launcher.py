...
class Lab3LauncherGUI:
    ...
    def _create_widgets(self):
        ...
        # Регрессия
        reg_frame = tk.LabelFrame(self.root, text="Регрессия (Блок 1)", font=("Arial", 11))
        reg_frame.pack(padx=20, pady=6, fill=tk.X)

        self._labeled_spin(reg_frame, "N:", 0, 0, 50, 2000, "280", attr='reg_n')
        self._labeled_spin(reg_frame, "ε₀:", 0, 2, 0.01, 2.0, "0.18", inc=0.01, attr='reg_eps')
        self._labeled_spin(reg_frame, "Epochs:", 1, 0, 50, 10000, "350", attr='reg_epochs')
        self._labeled_spin(reg_frame, "LR:", 1, 2, 0.001, 1.0, "0.02", inc=0.001, attr='reg_lr')
        ...
        # Классификация
        cls_frame = tk.LabelFrame(self.root, text="Классификация (Блок 2)", font=("Arial", 11))
        cls_frame.pack(padx=20, pady=6, fill=tk.X)

        self._labeled_spin(cls_frame, "N:", 0, 0, 50, 5000, "500", attr='cls_n')
        self._labeled_spin(cls_frame, "Шум:", 0, 2, 0.0, 3.0, "0.08", inc=0.01, attr='cls_noise')
        self._labeled_spin(cls_frame, "Epochs:", 1, 0, 50, 10000, "250", attr='cls_epochs')
        self._labeled_spin(cls_frame, "LR:", 1, 2, 0.001, 1.0, "0.03", inc=0.001, attr='cls_lr')
        ...
        # Кросс-валидация
        cv_frame = tk.LabelFrame(self.root, text="Кросс-валидация", font=("Arial", 11))
        cv_frame.pack(padx=20, pady=6, fill=tk.X)

        self._labeled_spin(cv_frame, "K folds:", 0, 0, 2, 20, "5", attr='cv_k')
    ...
    def _labeled_spin(self, parent, label, row, col, from_, to, default, inc=1, attr=None):
        tk.Label(parent, text=label, font=("Arial", 10)).grid(
            row=row, column=col, sticky=tk.W, padx=8, pady=6)
        spin = tk.Spinbox(parent, from_=from_, to=to, increment=inc, width=10, font=("Arial", 10))
    ...
    def _run_script(self, script, block):
        ...
        try:
            subprocess.Popen([sys.executable, path], env=env, cwd=self.script_dir)
        except Exception as exc:
            messagebox.showerror("Ошибка", f"Не удалось запустить {script}:\n{exc}")
